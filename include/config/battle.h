...
#define B_CRIT_CHANCE               GEN_5      // Chances of a critical hit landing. See CalcCritChanceStage. Gen6+ chances guarantee that Farfetch'd and Sirfetch'd always get critical hits while holding a Leek and using high-crit ratio moves.
...
#define B_KNOCK_OFF_DMG             GEN_3      // In Gen6+, Knock Off deals 50% more damage when knocking off an item.
...
#define B_ILLUMINATE_EFFECT         GEN_8      // In Gen9+, Illuminate prevents accuracy reductions and ignores the target's evasion.
...
#define B_OVERWORLD_SNOW                GEN_8      // In Gen9+, overworld Snow will summon snow instead of hail in battle.
