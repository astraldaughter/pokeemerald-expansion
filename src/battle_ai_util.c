...
{
    if (((!IsMoldBreakerTypeAbility(battlerAtk, AI_DATA->abilities[battlerAtk]) && (defAbility == ABILITY_SHIELD_DUST || defAbility == ABILITY_INNER_FOCUS || defAbility == ABILITY_UNSTOPPABLE))
...
bool32 ShouldFakeOut(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    && (AI_DATA->abilities[battlerDef] == ABILITY_SHIELD_DUST || AI_DATA->abilities[battlerDef] == ABILITY_INNER_FOCUS || AI_DATA->abilities[battlerDef] == ABILITY_UNSTOPPABLE)))
        return FALSE;
...
bool32 ShouldSetScreen(u32 battlerAtk, u32 battlerDef, u32 moveEffect)
{
    ...
    case EFFECT_AURORA_VEIL:
        // Use only if AI doesn't already have Reflect, Light Screen or Aurora Veil itself active.
