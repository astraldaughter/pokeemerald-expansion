...
static const MoveSuccessOrderCancellers sMoveSuccessOrderCancellers[] =
{
    ...
    [CANCELLER_PANICKED] = CancellerPanicked,
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_BAD_DREAMS:
                ...
                effect++;
                break;
            // Commented out because I'm buffing Solar Power. 
            //    
...
static inline u32 CalcDefenseStat(struct DamageCalculationData *damageCalcData, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    if (gMovesInfo[move].effect == EFFECT_PSYSHOCK || IsBattleMovePhysical(move)
...
static inline uq4_12_t GetBurnOrFrostBiteModifier(struct DamageCalculationData *damageCalcData, u32 abilityAtk)
{
    ...
    if (gBattleMons[battlerAtk].status1 & STATUS1_BURN
        ...
        return UQ_4_12(0.5);
    if (gBattleMons[battlerAtk].status1 & (STATUS1_FROSTBITE | STATUS1_PANIC)
...
static inline uq4_12_t GetRootedModifier(u32 move, u32 battlerDef)
{
    if (GetBattleMoveType(move) == TYPE_GROUND && gStatuses3[battlerDef] & STATUS3_ROOTED)
        return UQ_4_12(0.5);
    else if (GetBattleMoveType(move) == TYPE_GROUND && gStatuses3[BATTLE_PARTNER(battlerDef)] & STATUS3_ROOTED)
        return UQ_4_12(0.75);
    return UQ_4_12(1.0);
}
