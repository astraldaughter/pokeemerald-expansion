...
static bool32 AccuracyCalcHelper(u32 move, u32 battler)
{
    ...
    if (HasWeatherEffect())
    {
        if ((moveEffect == EFFECT_THUNDER || moveEffect == EFFECT_RAIN_ALWAYS_HIT)
            ...
            effect = TRUE;
        else if ((gBattleWeather & B_WEATHER_SANDSTORM) && moveEffect == EFFECT_SAND_ALWAYS_HIT)
            effect = TRUE;
        if (effect)
            return effect;
    }
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    case ABILITY_VICTORY_STAR:
        calc = (calc * 110) / 100; // 1.1 victory star / illuminate boost
        break;
    case ABILITY_HUSTLE:
...
static const u32 sCriticalHitOdds[]     = {16,  8,  4,  2,   1}; // 1/X, Gens 3,4,5
