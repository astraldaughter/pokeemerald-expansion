...
SINGLE_BATTLE_TEST("Toxic Spikes do not poison airborne Pokemon")
{
    ...
    PARAMETRIZE { species = SPECIES_MURMURAVEN; airborne = TRUE; }
    PARAMETRIZE { species = SPECIES_MURMURAVEN; item = ITEM_IRON_BALL; airborne = FALSE; }
    PARAMETRIZE { species = SPECIES_MURMURAVEN; move1 = MOVE_GRAVITY; airborne = FALSE; }
    PARAMETRIZE { species = SPECIES_MURMURAVEN; move1 = MOVE_INGRAIN; airborne = FALSE; }
...
SINGLE_BATTLE_TEST("Toxic Spikes are removed by grounded Poison-type Pokémon on switch in")
{
    ...
    bool32 grounded;
    PARAMETRIZE { species = SPECIES_HOGWELD; grounded = TRUE; }
    ...
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_HOGWELD].types[0] == TYPE_POISON);
