...
SINGLE_BATTLE_TEST("Stealth Rock damage on switch in based on typing")
{
    ...
    PARAMETRIZE { species = SPECIES_SCORNET; divisor = 2; } // Fire / Flying
    PARAMETRIZE { species = SPECIES_PIDGEOT; divisor = 4; } // Flying
    PARAMETRIZE { species = SPECIES_HOPSTALK; divisor = 8; } // Normal dmg
...
{
    GIVEN {
        PLAYER(SPECIES_TOXREC) { Item(ITEM_EJECT_BUTTON); }
        PLAYER(SPECIES_TOXREC);
